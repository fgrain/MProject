...
namespace Invector.vCharacterController
{
    public class vThirdPersonAnimator : vThirdPersonMotor
    {
        ...
        public virtual void UpdateAnimator()
        {
            ...
            if (inputMagnitude != 0)
            {
                float animMagnitude = Mathf.SmoothDamp(animator.GetFloat(vAnimatorParameters.InputMagnitude), stopMove ? 0f : inputMagnitude, ref fVelocity, isStrafing ? strafeSpeed.animationSmooth : freeSpeed.animationSmooth);
                animator.SetFloat(vAnimatorParameters.InputMagnitude, animMagnitude);
            }
            else
            {
